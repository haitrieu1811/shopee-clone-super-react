...
const Cart = () => {
  ...
  return (
    <Link to={config.routes.cart} className='relative mr-3 md:mr-0 md:mr-14 md:mt-6'>
      <Poppover
        reference={
          <Fragment>
                ...
                <div>
                  {cartList.slice(0, MAX_PURCHASES).map((cartItem) => (
                    ...
                    >
                      ...
                      <div className='flex-1 truncate px-4 text-[15px]'>{cartItem.product.name}</div>
                      <div className='text-sm font-medium text-orange'>đ{formatCurrency(cartItem.product.price)}</div>
                    </Link>
                  ))}
                </div>
                <div className='flex items-center justify-between p-3'>
                  <div className='text-sm capitalize text-gray-500'>
                    {cartList.length > MAX_PURCHASES && `${cartList.length - MAX_PURCHASES} sản phẩm khác`}
                  </div>
                  <Link
